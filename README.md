...
不填写命令行日期时，脚本仍默认计算上周一到上周日。所有正式周报统一使用以下标题格式：
...
自动任务不使用无日期参数的默认“上一整周”，因为完整周报中的盈亏业务线汇总只支持当前月累计或历史完整月份。生成结果必须使用正式模板和 `周报｜拼多多｜YYYY-Wxx｜金博敏` 标题，并核验店铺概况 7 行、消费者体验分 7 行、广告情况 7 个店铺库和盈亏情况 11 行。运行失败时保留日志并在任务结果中报告，不输出 `.env`、账号、密码、Cookie 或 Token，也不猜测或继续写入残缺数据。
