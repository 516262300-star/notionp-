...
完整周报不填写命令行日期时，脚本默认使用“截止昨天、从截止日所在月份的 1 日开始”的单月区间；如果运行日是月初，则自动生成上月完整月份。`--overview-only` 和 `--consumer-only` 轻量回填模式不填写日期时，仍使用上一整周。所有正式周报统一使用以下标题格式：
...
重复运行同一归属周时，不会重复创建周报页面；如果页面存在但 7 个店铺内嵌数据库不齐，会自动补齐缺失的店铺表，并按最新业务口径更新已有盈亏行。
...
- 当前月：本月 1 日到昨天（早间生成口径）或本月 1 日到今天。早上页面数据大部分截止到昨天时，统计范围截止到昨天，例如 8 月 8 日早上读取 `8.1—8.7`，周报标题按截止日 8 月 7 日归入 `2026-W32`。
- 已结束月份：该月 1 日到月末。
...
Codex 个人工作台中的自动任务名称为 `拼多多完整周报生成`，每周一 09:00（Asia/Shanghai）运行。它会先计算：

- 截止日：昨天。
- 开始日：截止日所在月份的 1 日。
- 月初运行且昨天为上月月末时：生成上月完整月份。

然后执行带明确日期的完整命令，例如：

```powershell
.\.venv\Scripts\python.exe main.py --start-date 2026-08-01 --end-date 2026-08-09
```

自动任务显式传入上述单月日期，完整周报主脚本的无日期默认值也采用相同口径，避免盈亏业务线汇总读取跨月区间。生成结果必须使用正式模板和 `周报｜拼多多｜YYYY-Wxx｜金博敏` 标题，并核验店铺概况 7 行、消费者体验分 7 行、广告情况 7 个店铺库和盈亏情况 11 行。运行失败时保留日志并在任务结果中报告，不输出 `.env`、账号、密码、Cookie 或 Token，也不猜测或继续写入残缺数据。
...
- 源数据库按 `日期` 过滤本次选择的统计周期。
- 店铺概况与消费者体验分源页面按七家店和目标周六逐店查询；任一店对应板块字段全部缺失时停止执行，不写入该周报。
- 日期可通过桌面生成器或 `--start-date`、`--end-date` 参数选择；完整周报命令行不传日期时取截止昨天的当月区间，两个轻量回填模式不传日期时取上一整周。
