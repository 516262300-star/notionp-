...
不填写命令行日期时，完整周报和两个轻量回填模式都使用上一整周（上周一至上周日）。广告情况严格按这个周周期汇总；盈亏情况因利德仕业务线汇总只支持月份筛选，会在脚本内部另行使用截止周报结束日的当月累计区间，历史月份则使用整月。所有正式周报统一使用以下标题格式：
...
“有效销售 - 业务线汇总”只有月份筛选，没有开始日/截止日筛选。因此“盈亏情况”数据库（包括其中的广告字段）使用与有效销售一致的月度口径，只允许以下业务区间：

- 当前月：本月 1 日到周报结束日；每周一生成时即本月 1 日到昨天。例如 2026 年 8 月 10 日生成 8 月 3—9 日周报时，“广告情况”读取 `8.3—8.9`，“盈亏情况”读取 `8.1—8.9`。
- 已结束月份：该月 1 日到月末。

“广告情况”始终使用周报起止日期精确筛选；“盈亏情况”里的拼多多、淘宝和天猫广告则跟随上述盈亏月度区间，保证广告占比等指标分子分母口径一致。淘宝、天猫广告会遍历全部分页，天猫固定取系统店铺参数 `103`。
...
主脚本自动计算上一整周作为周报及广告周期，并单独计算盈亏支持的单月区间；例如 2026 年 8 月 10 日运行时，周报及广告为 `2026-08-03` 至 `2026-08-09`，盈亏为 `2026-08-01` 至 `2026-08-09`。生成结果必须使用正式模板和 `周报｜拼多多｜YYYY-Wxx｜金博敏` 标题，并核验店铺概况 7 行、消费者体验分 7 行、广告情况 7 个店铺库和盈亏情况 11 行。运行失败时保留日志并在任务结果中报告，不输出 `.env`、账号、密码、Cookie 或 Token，也不猜测或继续写入残缺数据。
