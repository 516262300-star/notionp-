...
不填写命令行日期时，完整周报和两个轻量回填模式都使用上一整周（上周一至上周日）。“广告情况”只展示拼多多，并严格按这个周周期汇总；“盈利情况”中的拼多多、淘宝、天猫广告及有效销售、发货利润统一使用截止周报结束日的当月累计区间，历史月份则使用整月。所有正式周报统一使用以下标题格式：
...
“有效销售 - 业务线汇总”只有月份筛选，没有开始日/截止日筛选。因此“盈利情况”中的拼多多、淘宝、天猫广告及有效销售、发货毛利、发货净利全部使用同一个月度口径，只允许以下业务区间：

- 当前月：本月 1 日到周报结束日；每周一生成时即本月 1 日到昨天。例如 2026 年 8 月 10 日生成 8 月 3—9 日周报时，“广告情况”读取拼多多 `8.3—8.9`，“盈利情况”的拼多多、淘宝、天猫广告及全部利润指标读取 `8.1—8.9`。
- 已结束月份：该月 1 日到月末。

“广告情况”的拼多多源数据库使用周报起止日期精确筛选；“盈利情况”里的拼多多、淘宝和天猫广告跟随盈利月度区间，保证 `广告占比 = 当月累计广告费 ÷ 当月累计有效销售` 的分子分母一致。淘宝、天猫广告会遍历全部分页，天猫固定取系统店铺参数 `103`。
...
主脚本自动计算上一整周作为周报及“广告情况”周期，并单独计算“盈利情况”的单月区间；例如 2026 年 8 月 10 日运行时，“广告情况”的拼多多数据为 `2026-08-03` 至 `2026-08-09`，“盈利情况”的拼多多、淘宝、天猫广告及利润指标为 `2026-08-01` 至 `2026-08-09`。生成结果必须使用正式模板和 `周报｜拼多多｜YYYY-Wxx｜金博敏` 标题，并核验店铺概况 7 行、消费者体验分 7 行、广告情况 7 个店铺库和盈利情况 11 行。运行失败时保留日志并在任务结果中报告，不输出 `.env`、账号、密码、Cookie 或 Token，也不猜测或继续写入残缺数据。

个人工作台任务已同步 ERP 有效销售明细的月底容错规则：该明细单次最多等待 120 秒，读取超时后按 2 秒、4 秒间隔再试 2 次；自动任务必须等待脚本自身重试完成，只有 3 次尝试全部失败后才报告 ERP 明细超时。
...
- 为兼容尚未刷新的旧个人工作台提示，每周一若完整命令仍显式传入“当月 1 日至昨天”，主脚本会自动纠正出上一整周作为周报及“广告情况”周期；“盈利情况”仍按当月 1 日至周报截止日读取全部广告和利润指标。
- 盈亏数据会在创建 Notion 页面前先完整抓取；网站登录失效或字段解析失败时不会创建半成品周报。
- ERP“有效销售 - 业务线汇总”的店铺展开明细在月底可能因数据量大而返回较慢；该明细请求单独使用 120 秒读取超时，读取超时后最多再重试 2 次（间隔 2 秒、4 秒）；其他 ERP 请求仍保持 45 秒。若 3 次尝试仍失败，查看最新日志中的业务线和月份，待 ERP 页面恢复后重新生成。
